...
sql_str = (
	...
	SELECT *  INTO movies_top FROM (
		SELECT movieId, COUNT(rating) AS ratings_num, AVG(ratings_num) AS ratings_avg
		FROM ratings
		GROUP BY movieId 
		HAVING AVG(ratings_num) > 3
	);
	"""
)
...
top_rated_query = session.query(MoviesTop.movieId).filter(MoviesTop.ratings_num>15).filter(MoviesTop.ratings_avg>3.5).order_by(MoviesTop.ratings_avg.desc())
top_rated_content_ids = [i for i in top_rated_query]
...
top_5_tags = tags_df.groupby('tags').count()['movieid'].head(5)
